...
public class Panel extends JPanel {
    ...
    public Panel() {
        ...
        setSize(800,520);
        try {
            ranen = ImageIO.read(Objects.requireNonNull(getClass().getResource("image/ranen.png")));
            boom = ImageIO.read(Objects.requireNonNull(getClass().getResource("image/boom.png")));
            killed = ImageIO.read(Objects.requireNonNull(getClass().getResource("image/killed.png")));
            paluba = ImageIO.read(Objects.requireNonNull(getClass().getResource("image/paluba.png")));
    ...
    public void paintComponent(Graphics g) {
        ...
        g.setFont(new Font("Comic Sans MS", 0, H-6));
        g.setColor(new Color(234, 22, 150));
        if (rasstanovka) {
        ...
        g.drawString("Ходов комьютера: ", DXY + 23 * H, DXY + 14 * H - (H/4));
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                ...
                if (game.masComp[i][j]!=0) {
                    ...
                    if ((game.masComp[i][j] >= 8) && (game.masComp[i][j] <= 11)) {
        ...
        for (int i = DXY; i <= DXY + 10 * H; i += H) {
            ...
            g.drawLine(DXY + 13 * H, i, DXY + 23 * H, i);
            g.drawLine(i + 13 * H, DXY, i + 13 * H, DXY + 10 * H);

            g2.setStroke(new BasicStroke(2));
            g.setColor(new Color(0xEA1696));
            g.drawRect(DXY, DXY, 10 * H, 10 * H);
            g.drawRect(DXY+13*H,DXY,10 * H,10 * H);
        }

        g.setFont(new Font("Comic Sans MS", 0, H));
        g.setColor(new Color(0xEA1696));

        //количество кораблей пк
        ...
        g.drawRect(DXY + 13 * H, DXY + 11 * H, 4 * H, H);
        ...
        if (Game.endGame==0 && (p1+p2+p3+p4)==0 && rasstanovka || Game.endGame==0 && !rasstanovka){
            g.setFont(new Font("Comic Sans MS", 0, H - 5));
            if (game.myHod) {
    ...
    public class Mouse implements MouseListener,MouseMotionListener {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            if ((e.getButton() == 1) && (e.getClickCount() == 1)) {
                mX = e.getX();
                mY = e.getY();
                if ((rasstanovka && p1+p2+p3+p4==0) || !rasstanovka
